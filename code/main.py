...
def generate(data = Body()):
    ...
    #выдать файл
    if res == True:
        return FileResponse(f'./data/TKPexample.xlsx', filename=f'ТКП ПК.xlsx', media_type='application/xlsx', status_code=200, headers = ['Content-Disposition', 'attachment'])
    else:
        return res
...
def mk_OL(data = Body()):
    ...
    #выдать файл
    if res == True:
        return FileResponse(f'./data/OLexample.xlsx', filename=f'ОЛ ПК.xlsx', media_type='application/xlsx', status_code=200, headers = ['Content-Disposition', 'attachment'])
    else:
        return res
